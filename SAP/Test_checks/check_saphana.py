...
daemon1=os.popen("/usr/sap/hostctrl/exe/sapcontrol -nr 00 -function GetProcessList | grep hdbindexserver | awk '{print $1}'").readline().strip()

# With original SAP HANA Command, we can check status of the process

status1=os.popen("/usr/sap/hostctrl/exe/sapcontrol -nr 00 -function GetProcessList | grep hdbindexserver | awk '{print $4}'").readline().strip()

# Checking actual process ID

id1=os.popen("/usr/sap/hostctrl/exe/sapcontrol -nr 00 -function GetProcessList | grep hdbindexserver | awk '{print $11}'").readline().strip()
