...
daemon1=os.popen("/usr/sap/hostctrl/exe/sapcontrol -nr 00 -function GetProcessList | grep sapwebdisp_hdb | awk '{print $1}'").readline().strip()

# With original SAP HANA Command, we can check status of the process

status1=os.popen("/usr/sap/hostctrl/exe/sapcontrol -nr 00 -function GetProcessList | grep sapwebdisp_hdb | awk '{print $4}'").readline().strip()

# Checking actual process ID

id1=os.popen("/usr/sap/hostctrl/exe/sapcontrol -nr 00 -function GetProcessList | grep sapwebdisp_hdb | awk '{print $11}'").readline().strip()


if (status1 == "GREEN," and daemon1 == "sapwebdisp_hdb,"):
        print "Process %s it's running in process ID : %s - process status : %s." % (daemon1, id1, status1)
        sys.exit(0)

if (status1 == "YELLOW," and daemon1 == "sapwebdisp_hdb,"):
        print "WARNING - Process %s is %s it's going up" % (daemon1, status1)
        sys.exit(1)

elif (status1 == "GRAY" and daemon1 == "sapwebdisp_hdb,"):
